...
#define DEVICETE_MAX_CMD_LEN	10

namespace controlbox {
namespace device{

/// A SampiTE is a DeviceTE for SAMPI's "Testate Elettroniche".
/// This class provide access to SAMPI's TE devices connected to
...
class SampiTE : public DeviceTE  {
  ...
  public:
	...
	SampiTE(std::string const & logName = "SampiTE");
  ...
  protected:
	...
	/// A TE command
	struct teCmd {
		char cmd [DEVICETE_MAX_CMD_LEN];	///> The HEX rapresentation of the TE command
...
// 	std::string formatEvent(t_event const & event);
	...
	inline DeviceTE::t_cmdType cmdTypeGenericCode() {
		return DeviceTE::SEND_GENERIC_SAMPI;
	};
	...
	inline SampiTE::t_teResps checkResponce(const char * buf);
