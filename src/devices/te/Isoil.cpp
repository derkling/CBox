...
Isoil::Isoil(std::string const & logName) :
	DeviceTE(DeviceTE::ISOIL, logName) {
	LOG4CPP_DEBUG(log, "Isoil(std::string const &)");
}

Isoil::~Isoil() {
	LOG4CPP_DEBUG(log, "Destroying the Isoil device");
}



inline exitCode
Isoil::checksum(const char * buf, unsigned len, char chksum[3], bool tx) {
	unsigned sum = 0;
	unsigned index = 0;
...
bool
Isoil::verifyChecksum(const char * buf, unsigned len) {
	char rxChksum[] = "\0\0\0";
	char myChksum[] = "\0\0\0";
	bool match = false;
...
exitCode
Isoil::downloadEvents(t_eventList & eventList) {
/*
	unsigned len, index;
	unsigned short retry;
	char buf[DEVICE_TE_SAMPI500_RX_BUFFER];
	Isoil::t_teResps resp;
	unsigned device;
...
exitCode
Isoil::formatEvent(t_event const & p_event, comsys::Command & cmd) {
